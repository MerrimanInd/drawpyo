#!/usr/bin/env python3
"""
Create a more comprehensive Azure architecture diagram with imported shapes.
"""

import drawpyo

def main():
    print("Creating Azure Architecture Diagram...")
    print("-" * 60)
    ...
    print(f"Found {len(shape_names)} Azure shapes")
    print(f"Creating diagram with {min(3, len(shape_names))} shapes...\n")
    ...
    # Connect the icons with arrows if we have multiple
    if len(icons) > 1:
        for i in range(len(icons) - 1):
            ...
            edge.strokeWidth = 2
        print(f"\n  ✓ Connected {len(icons) - 1} edges")
    
    # Save
    file.write()
    print(f"\n✓ Diagram saved to: {file.file_path}/{file.file_name}")
    print("-" * 60)
    print("You can now open the file in Draw.io to see the Azure icons!")
